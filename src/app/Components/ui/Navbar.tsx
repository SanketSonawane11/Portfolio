  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "flex max-w-fit fixed top-10 inset-x-0 mx-auto border border-white/[0.2] rounded-3xl bg-black shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] pr-8 pl-8 py-6  items-center justify-center space-x-4",
          className
        )}
      >
        {navItems.map((navItem: any, idx: number) => (
          <Link
            ...
            href={navItem.link}
            className={cn(
              "relative items-center flex space-x-1 text-neutral-50 hover:text-neutral-300 hover:scale-105 rounded-lg duration-150 ease-out transition-all hover:font-semibold"
            )}
          >
        ...
        <div className="flex items-center justify-center gap-4">
          {socialLinks.map((i) => (
            <a
              ...
              href={i.link}
              target="_blank"
              className="flex items-center space-x-2 text-neutral-50 hover:text-neutral-300 hover:scale-105 duration-150 ease-out transition-all hover:font-semibold"
            >
              <span className="block active:bg-gradient-to-r active:from-blue-500 active:to-purple-500 active:bg-clip-text active:text-transparent transition-all ease-out duration-200 sm:hidden">
                {i.icon}
              </span>
